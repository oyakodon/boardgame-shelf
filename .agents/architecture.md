...
- ゲームの基本情報はメンバーの手入力とする。BGGとBGA(ボードゲームアリーナ)のリンクは任意の補助情報として持つが、自動取得はしない。どちらもURLをそのまま保存せず、IDまたはスラッグだけを保存してURLは表示時に組み立てる(フォームにゲームページのURLを貼り付けてもIDだけ抽出する)

会場でスマートフォンから開く使い方を主とする。
単一のCloudflare Workerが、静的アセット(React SPA)とAPIの両方を配信する。
